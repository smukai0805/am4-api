...
// /players/squads?team=541 に「Castilla/下部組織所属で今季トップチームの公式戦に
// 一度も出場していない選手」が混ざっていることが判明した(例: 20歳のGKが2試合とも
// 0出場のままトップの背番号帯に登録されている)。一方で、直近の移籍市場(2026年
// 6〜7月)でレアル・マドリードへ加入したばかりの新戦力(イブラヒマ・コナテ、
// マルク・ククレジャ、デンゼル・ダンフリース、ベルナルド・シルバ、カルロス・エスピ等)
// も直近シーズンの出場数は0だが、これらは/transfersで実際に「Real Madridへの移籍」が
// 確認できる正規の新戦力であり、除外してはいけない。
//
...
//      /transfers の中で最も日付が新しいレコードを探し(配列は新しい順とは限らない
//      ため要注意、実データでも古いレコードが先頭に来るケースを確認済み)、その
//      移籍先(teams.in.id)がこのクラブと一致し、かつ移籍元(teams.out.id)が
//      このクラブ自身ではない(=他クラブからの正規加入である)場合のみ、
//      「今シーズンの新加入選手」としてトップチーム所属とみなす。move元・先が
//      共にこのクラブ自身になっているレコード(下部組織内の登録更新等、クラブ内の
//      移動と思われるもの)は対象外とする
