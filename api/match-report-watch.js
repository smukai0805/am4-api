...
import '../lib/blob-environment.js';
...
function buildMatchInfo(fixture, events = []) {
  return {
    ...
    sourceReferences: [
      {
        title: `API-Football契約データ（fixture ID ${fixture.fixture.id}: 結果・試合状態）`,
        url: `https://v3.football.api-sports.io/fixtures?id=${encodeURIComponent(fixture.fixture.id)}`,
      },
      {
        title: `API-Football契約データ（fixture ID ${fixture.fixture.id}: イベント）`,
        url: `https://v3.football.api-sports.io/fixtures/events?fixture=${encodeURIComponent(fixture.fixture.id)}`,
      },
      {
        title: `API-Football契約データ（fixture ID ${fixture.fixture.id}: 選手スタッツ）`,
        url: `https://v3.football.api-sports.io/fixtures/players?fixture=${encodeURIComponent(fixture.fixture.id)}`,
      },
    ],
...
// API-Footballの検証済み結果・イベント・選手スタッツをそのまま使う決定的な下書き生成。
// Notion同期系と同じ関数上限を維持し、API-Footballの一時的な遅延にも安全に対応する。
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const matchInfo = buildMatchInfo(fixture, events);
      const tGen = Date.now();
  ...
  try {
    ...
    // 記事生成（採点算出と決定的な本文構成）は、上流負荷とNotion同期を有限に保つため
    // 1回あたり上限試合数までにする。
