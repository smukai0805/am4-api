...
const MANUAL_REVIEW_OUTCOMES = new Set([
  ...
  'insufficient_identity', 'not_found', 'fixture_missing',
]);
...
async function rollbackSnapshots(snapshots, sourceVersionValue, dependencies, ensureWriteOwnership) {
  const result = [];
  for (const snapshot of [...snapshots].reverse()) {
    ...
    try {
      ...
      await dependencies.saveArticle(snapshot.before);
      result.push({ articleId: snapshot.articleId, state: 'restored' });
    } catch (error) {
...
async function reserveReportGenerationAttempt(job, context) {
...
async function updateReportGenerationAudit(store, fixture, patch, now) {
  ...
  try {
    await store.updateState((state) => {
      const repair = state.matchReportRepair && typeof state.matchReportRepair === 'object'
        ? state.matchReportRepair : {};
...
async function processReportGeneration(job, context) {
  ...
  if (prepared.state === 'ready') {
    ...
    const reservation = await reserveReportGenerationAttempt(job, context);
  ...
  const verification = await store.enqueue({
    kind: 'article_validation',
    articleId: delivery.article.id,
    sourceType: 'match_report',
    sourceVersion: delivery.sourceVersion || sourceVersionValue,
    deliveryOnly: false,
...
async function processArticle(job, context) {
  ...
  if (job.kind === 'notion_page') {
    const persistedArticle = await readCurrentPersistedNotionPage(job, dependencies);
    const alreadyDelivered = await deliveredNotionPagePreflight(job, dependencies, persistedArticle);
...
async function handleResult(job, result, context) {
  ...
  if (result.state === 'verification_failed' || result.state === 'browser_failed') {
    ...
    // in-memory result. A browser failure can be retried in a new Function,
    // so the final bounded rollback must include writes from every attempt.
    const persistedSnapshots = await store.readSnapshots(job.id);
    const snapshots = [...new Map(
      [...persistedSnapshots, ...(result.snapshots || [])].map((snapshot) => [snapshot.id, snapshot]),
    ).values()];
    const rolledBack = snapshots.length
      ? await rollbackSnapshots(snapshots, result.sourceVersion || job.sourceVersion, dependencies, ensureWriteOwnership)
      : [];
    const finished = await store.finishJob(job.id, {
      owner,
  ...
  if (result.state === 'browser_unavailable') {
    const finished = await store.finishJob(job.id, { owner, status: 'blocked', result, error: result.browser?.reason || 'browser_unavailable' });
    if (!finished) return { status: 'lease_lost', result };
    await alert(store, dependencies, {
