...
export const TRANSIENT_BROWSER_RUNTIME_RECOVERY_GENERATION = 'browser-runtime-interruption-recovery-v1';
const TRANSIENT_BROWSER_RUNTIME_FAILURE = 'runtime_browser_transport_interrupted';
// The ordinary editorial worker remains capped at twenty browser launches per
// Tokyo day. Two already-held, code-owned recoveries from the explicit
// Chromium-interruption incident need a real browser result now rather than a
// new midnight delay. This is deliberately a finite ceiling, not a ledger
// reset: only this exact job kind can reserve launch 21 or 22.
const TRANSIENT_BROWSER_RUNTIME_RECOVERY_BROWSER_LIMIT = 22;
...
// This job shape is produced only by the migration below after it has
// re-read the exact private Notion page and version. Keep its provenance
// predicate beside the job definition so quota-policy release and browser
// reservation cannot accidentally apply to a future generic job using the
// same broad kind name.
export function isTransientBrowserRuntimeRecoveryJob(job) {
  return job?.kind === TRANSIENT_BROWSER_RUNTIME_RECOVERY_KIND
    && job?.repairGeneration === TRANSIENT_BROWSER_RUNTIME_RECOVERY_GENERATION
    && job?.trigger === 'transient_browser_runtime_recovery'
...
export async function migrateClaimQueueMetadata(store, sourceTypes, kinds, now) {
  if (
...
async function browserInspection(article, fixture, store, dependencies, limits, {
  deadlineAt = null,
  now = () => new Date(),
  browserLaunchLimit = null,
} = {}) {
  ...
  const elevatedBrowserLaunchLimit = Number(browserLaunchLimit);
  const browserLimits = Number.isFinite(elevatedBrowserLaunchLimit) && elevatedBrowserLaunchLimit > 0
    ? {
      ...limits,
      maxBrowserLaunchesPerDay: Math.max(
        Number(limits.maxBrowserLaunchesPerDay) || 0,
        Math.floor(elevatedBrowserLaunchLimit),
      ),
    }
    : limits;
  const quota = await reserve(store, browserLimits, { browserLaunches: 1 });
...
async function syncPageArticle(job, context) {
  ...
  const outcome = await dependencies.syncPage({
    ...
    releaseMonitorDeliveryHold: isTransientBrowserRuntimeRecoveryJob(job),
    articleStore,
...
async function processArticle(job, context) {
  ...
  const browser = await browserInspection(article, fixture, store, dependencies, limits, {
    deadlineAt,
    now,
    browserLaunchLimit: isTransientBrowserRuntimeRecoveryJob(job)
      ? TRANSIENT_BROWSER_RUNTIME_RECOVERY_BROWSER_LIMIT
      : null,
  });
...
async function enqueueLegacyTransientBrowserRuntimeRecoveries(store, dependencies) {
  ...
  if (inspectedLegacyJobIds.length) {
    await store.updateState((current) => {
      const currentRecovery = current.browserRuntimeRecovery && typeof current.browserRuntimeRecovery === 'object'
        ? current.browserRuntimeRecovery
        : {};
