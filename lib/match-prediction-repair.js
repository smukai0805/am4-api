...
const PREDICTION_URGENT_WINDOW_MS = 36 * 60 * 60 * 1000;
// Changing this code-owned value allows one safe re-observation of an
// existing durable queue when the urgency rule itself changes. It never
// creates a second source job for the same fixture/version.
const PREDICTION_QUEUE_PRIORITY_VERSION = 'prediction-kickoff-priority-v1';
...
function scanAllowed(state, now) {
  ...
  if (prior.queuePriorityVersion !== PREDICTION_QUEUE_PRIORITY_VERSION) return true;
  return !(
    date
...
function scheduledAt(candidate) {
  ...
  // A date-only editorial card is a Japan-facing schedule. Never leave this
  // to the server's local timezone when ordering the durable queue.
  return date ? Date.parse(`${date}T12:00:00+09:00`) : Number.POSITIVE_INFINITY;
}

function predictionQueuePriority(candidate, observed) {
  const kickoff = scheduledAt(candidate);
  const observedAt = new Date(observed).getTime();
  // Upcoming matches within the next 36 hours must be generated before the
  // following day's backlog. A stale provider NS/TBD status remains
  // provider-authoritative, but is also urgent for repair rather than being
  // silently skipped based only on elapsed time.
  if (Number.isFinite(kickoff) && Number.isFinite(observedAt) && kickoff - observedAt <= PREDICTION_URGENT_WINDOW_MS) return 98;
  return 94;
}
...
export async function scanMissingMatchPredictions({
  ...
  const missing = [...candidates.values()]
    .filter((candidate) => !rows.some((article) => isPublicPrediction(article, candidate)))
    .sort((left, right) => scheduledAt(left) - scheduledAt(right) || left.id - right.id);
  const queued = [];
  ...
  for (const candidate of missing) {
    const item = await store.enqueue({
      ...
      trigger: 'scheduled_fixture_scan', priority: predictionQueuePriority(candidate, observed),
      // Re-observing a queued fixture must be able to promote a match whose
      // kickoff is now near, without changing its durable identity or making
      // another Notion page.
  ...
  await store.updateState((state) => {
    ...
    return {
      ...
      matchPredictionRepair: {
        ...
        queuePriorityVersion: PREDICTION_QUEUE_PRIORITY_VERSION,
