...
export async function listArticles({ type, matchDate, fixtureId, search, page = 1, pageSize = 10, includeHidden = false, publishedOnly = false } = {}) {
  const index = await loadIndex();
  const visible = includeHidden ? index : index.filter((entry) => entry.public !== false);
  const publiclyVisible = publishedOnly ? index.filter(isPublicArticle) : visible;
  ...
  const filtered = publiclyVisible.filter((entry) => (
