...
import { articleCoverImage, articleMatchesSearch, articlePopularRank, articlePriority, articleSearchText, articleTags, normalizeArticleSearch } from './article-taxonomy.js';
...
async function searchArticleMetadata(entries, search, options) {
  const matches = [];
  // The index now retains a server-only search text field when an article is
  // saved. Older entries do not have it, so only those legacy candidates need
  // an individual Blob read. Keep reads bounded in small batches.
...
export async function saveArticle(article) {
  ...
  const meta = { ...articleMetadata(article), searchText: articleSearchText(article) };
