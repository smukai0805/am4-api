...
function stateDefault() {
  return {
    ...
    // A one-time, code-owned migration record.  It prevents each Production
    // deployment from repeatedly reopening the newest legacy browser-runtime
    // hold while older, still eligible records never get a turn.
    browserRuntimeRecovery: {
      generation: null,
      processedLegacyJobIds: [],
      browserQuotaPolicyVersion: null,
      browserQuotaPolicyAppliedAt: null,
    },
    lastRun: null,
...
function normaliseState(value) {
  ...
  const browserRuntimeRecovery = safeObject(source.browserRuntimeRecovery);
  return {
    ...stateDefault(),
    ...source,
    collector: {
    ...
    browserRuntimeRecovery: {
      generation: typeof browserRuntimeRecovery.generation === 'string'
        ? browserRuntimeRecovery.generation.slice(0, 120)
        : null,
      // Queue history itself is bounded to 500 records.  Retaining this
      // matching bound is enough to progress through every recoverable legacy
      // record without allowing state growth from arbitrary input.
      processedLegacyJobIds: Array.isArray(browserRuntimeRecovery.processedLegacyJobIds)
        ? [...new Set(browserRuntimeRecovery.processedLegacyJobIds
          .filter((jobId) => typeof jobId === 'string' && /^[a-f0-9]{40}$/iu.test(jobId)))].slice(-HISTORY_LIMIT)
        : [],
      browserQuotaPolicyVersion: typeof browserRuntimeRecovery.browserQuotaPolicyVersion === 'string'
        ? browserRuntimeRecovery.browserQuotaPolicyVersion.slice(0, 120)
        : null,
      browserQuotaPolicyAppliedAt: typeof browserRuntimeRecovery.browserQuotaPolicyAppliedAt === 'string'
        ? browserRuntimeRecovery.browserQuotaPolicyAppliedAt.slice(0, 80)
        : null,
    },
  };
}
...
// Historical monitor versions treated these explicit Playwright/Chromium
// process failures as reader-visible regressions. Only this narrow legacy
// marker is eligible for the code-owned recovery migration; ordinary visual
// failures and a generic word such as "closed" must remain terminal.
function isLegacyTransientBrowserRuntimeError(value) {
  const text = String(value || '').toLowerCase();
  return /target page, context or browser has been closed|browser (?:has been )?closed|browser disconnected|browser.*(?:closed unexpectedly|disconnected)|page (?:has been )?crashed|target crashed|context (?:has been )?closed|protocol error.*(?:closed|disconnect|crash|connection (?:closed|reset))|(?:closed|disconnect|crash).*protocol|connection (?:closed|reset).*browser|browser.*connection (?:closed|reset)/i.test(text);
}
...
export function createSiteMonitorStore({
  ...
  async function enqueue(input = {}) {
    ...
    let job = existing.value || initial;
    const requestedPriority = positiveInteger(input.priority);
    let priorityPromoted = false;
    // A source-controlled scheduler may re-observe an already queued job as
    // more urgent (for example, a match entering its near-kickoff window).
    // Promote only an unleased queued record; terminal jobs and live workers
    // keep their exact identity/state, so this cannot reopen work or race a
    // writer. Persist it on the authoritative job as well as the queue
    // projection because later quota requeues rebuild from the job priority.
    if (
      input.promotePriority === true
      && job.status === 'queued'
      && !job.leaseOwner
      && requestedPriority > positiveInteger(job.priority)
    ) {
      const promoted = await updateJob(id, (current) => {
        if (
          !current
          || current.status !== 'queued'
          || current.leaseOwner
          || requestedPriority <= positiveInteger(current.priority)
        ) return null;
        return { ...current, priority: requestedPriority };
      });
      if (promoted.changed && promoted.value) {
        job = promoted.value;
    ...
    if (['completed', 'blocked', 'failed'].includes(job.status)) return { job, enqueued: false, duplicate: true, priorityPromoted };
    ...
    await updateQueue((queue) => {
      ...
      if (present) {
        ...
        const queuePriorityPromoted = input.promotePriority === true
          && !present.leaseOwner
          && requestedPriority > positiveInteger(present.priority);
        if (
          (!present.sourceType && sourceType)
          || (!present.kind && kind)
          || (deliveryOnly && present.deliveryOnly !== true)
          || queuePriorityPromoted
        ) {
          ...
          return {
            ...
            items: queue.items.map((item) => item.jobId !== id ? item : {
              ...
              ...(queuePriorityPromoted ? { priority: requestedPriority } : {}),
            }),
          };
        }
        return queue;
    ...
    return { job, enqueued: added, duplicate: !added, metadataMigrated, priorityPromoted };
  }
  ...
  async function requeueDeferredUsageLimitedJobs({
    ...
    const allowedSourceTypes = Array.isArray(sourceTypes)
      ? new Set(sourceTypes.filter((sourceType) => typeof sourceType === 'string' && sourceType))
      : null;
    const allowedKinds = Array.isArray(kinds)
      ? new Set(kinds.filter((kind) => typeof kind === 'string' && kind))
      : null;
    const match = typeof matchesJob === 'function' ? matchesJob : null;
    ...
    const candidates = queue.value.items
      ...
      // Queue metadata is only a projection. Scan a small bounded window and
      // re-check the authoritative job below so a malformed same-kind record
      // cannot consume a code-owned recovery slot ahead of a valid one.
      .slice(0, 50);
    const releaseLimit = Math.min(50, Math.max(1, positiveInteger(limit, 50)));
    const released = [];
    for (const item of candidates) {
      if (released.length >= releaseLimit) break;
      const updated = await updateJob(item.jobId, (job) => {
        if (
          !job
          || job.status !== 'queued'
          || job.leaseOwner
          || (allowedSourceTypes && !allowedSourceTypes.has(job.sourceType))
          || (allowedKinds && !allowedKinds.has(job.kind))
          || (match && match(job) !== true)
  ...
  // Return only records that the pre-runtime-classification monitor
  // mislabelled as a visual failure. Callers must create a fresh, code-owned
  // recovery job rather than reopening this terminal job: it may contain an
  // old rollback lineage and exhausted attempt counter.
  async function findBlockedTransientBrowserValidationJobs({ limit = 20, excludeJobIds = [] } = {}) {
    const queue = await readQueue();
    const safeLimit = Math.min(50, Math.max(1, positiveInteger(limit, 20)));
    const excluded = new Set(Array.isArray(excludeJobIds)
      ? excludeJobIds.filter((jobId) => typeof jobId === 'string' && jobId)
      : []);
    const candidateIds = [...new Set(queue.value.history
      .filter((entry) => entry?.status === 'blocked' && entry?.error === 'browser_failed')
      .map((entry) => entry.jobId)
      .filter((jobId) => typeof jobId === 'string' && jobId && !excluded.has(jobId))
      .slice(-safeLimit)
      .reverse())];
    const jobs = [];
    for (const jobId of candidateIds) {
      ...
      if (
        job
        && job.status === 'blocked'
        && job.kind === 'article_validation'
        && job.lastError === 'browser_failed'
        && job.result?.state === 'browser_failed'
        && isLegacyTransientBrowserRuntimeError(job.result?.browser?.error)
      ) jobs.push(job);
    }
    return { jobs };
  }
  ...
  return {
    ...
    findBlockedTransientBrowserValidationJobs,
    acquireLock,
    renewLock,
    releaseLock,
    recordEvent,
