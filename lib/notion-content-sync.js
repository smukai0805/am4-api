...
// A generated draft must be editorially advanced before it is exposed through
// the public archive. `自動生成` remains a useful Notion workflow state, but is
// deliberately not a public state.
  ...
  for (const source of resolveSourceDefinitions(sourceIds)) {
    try {
      ...
      for (const page of pages) {
        ...
        if (!isPublishableNotionState(state)) {
          // `自動生成` is not a state from which this synchronizer may create
          // a new public record. Older archive entries, however, used it as
          // source provenance while their explicit archive publication had
          // already been approved. Do not erase that legacy archive merely
          // because a later sync sees the old provenance label. Explicit
          // review/non-public states still retract an existing mirror.
          if (
            existingArticle
