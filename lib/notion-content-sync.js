...
function markdownEditorialSections(markdown) {
  ...
  String(markdown || '').replace(/\r\n?/g, '\n').split('\n').forEach((line) => {
    ...
    if (heading) {
      ...
      // H3/H4 subsections inside an H2 editorial field belong to that field.
      // Treat only a same-or-higher heading as the next top-level section, so
      // authored key-player headings such as "### Elye Wahi（Nice）" remain
      // inside the structured "## キーマン" value.
    ...
    if (calloutHeading) {
      flush();
      // The summary callout closes when the next Markdown heading appears.
      current = { heading: calloutHeading[1], body: '', level: 6 };
      return;
    }
