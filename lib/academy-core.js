...
1. 見出し(読者の興味を引くキャッチコピー形式にすること。選手の特徴・出身国・
   ...
   読みたくなる見出しにすること。
   ★★技術的な形式(必ず厳守): この見出しは、記事本文の1行目に「# 」(半角シャープ+
   半角スペース)で始まる、Markdownの見出し1行だけで書くこと。その1行にはキャッチ
   コピー本文だけを書き、選手名・ポジション・クラブ名を前置き・接頭辞として付け
   加えないこと(例:「# J. Ekhator(FW)|新星の輝き」のように機械的な表記とキャッチ
   コピーを組み合わせるのも不可。キャッチコピーのみで完結させること)。この1行を
   後続処理が機械的に見出しとして抽出するため、この形式を外れると正しく抽出できない。
   ...
   悪い例(絶対に避ける):
     ...
     「Kobbie Mainoo選手紹介」
     「J. Ekhator(FW)|ユヴェントス期待の19歳、プレシーズンで見せた190cmの長身アタッカー」)
...
export async function generateArticleDraft(detectedPlayer, profile, { client = callAnthropic } = {}) {
...
`;
  // 調査なしの選手紹介を下書きとして残さない。失敗時はwatcherが候補を未処理のまま
...
- 見出し(1.)は必ず読者の興味を引くキャッチコピー形式にすること。「${detectedPlayer.name}(${detectedPlayer.club})」のような選手名・クラブ名だけの機械的な見出しは絶対に使わないこと。記事本文1行目は「# 」+キャッチコピー本文のみの1行にすること(選手名・ポジション・クラブ名を前置きしない)。詳しい要件はフォーマット仕様の1.を参照。
...
// api/academy-debut-watch.js・api/transfer-news-watch.jsで重複していたロジックを
// 集約した(2026-08-04)。
//
// 【2026-08-04の実データ調査で判明】プロンプトで「読者の興味を引くキャッチコピー」
// 「# で始まる1行だけで書く」と指示していても、モデルがこの形式を守らないことが
// あり(見出し行自体が存在しない、または「J. Ekhator(FW)|ユヴェントス期待の19歳...」
// のように機械的な名前表記とキャッチコピーを混在させてしまう)、その場合は無言で
// fallback(選手名+クラブ名の機械的な表記)が使われ、「O. Vargas(Manchester United)」
// のようなタイトルになっていた。
// 対策として、抽出した見出し候補が日本語を含まない場合(=機械的な英字名+クラブ名
// 表記である可能性が高い)は候補から除外する。また、見出しに「|」が含まれ、その
// 手前部分に日本語が無い場合(=機械的な名前表記+キャッチコピーの混在)は、「|」より
// 後ろのキャッチコピー部分だけを採用する。
function containsJapanese(s) {
  return /[぀-ゟ゠-ヿ一-鿿]/.test(s);
}
export function extractTitle(draft, fallback) {
  const headings = [...(draft || '').matchAll(/^#{1,3}\s+(.+)$/gm)].map(m => m[1].trim());
  let real = headings.find(h => !h.includes('フォーマット(AM4)') && containsJapanese(h));
  if (real) {
    const pipeIdx = real.indexOf('|');
    if (pipeIdx !== -1 && !containsJapanese(real.slice(0, pipeIdx))) {
      real = real.slice(pipeIdx + 1).trim();
    }
  }
