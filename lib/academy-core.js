...
`;
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      // match-report-core.js側の実データ検証でstop_reason:"max_tokens"により記事本文が
      // 生成される前に打ち切られる事象を確認した(Web検索5回分の結果で入力が20万トークン超
      // になり、thinking+本文の生成分がmax_tokens内に収まらなかった)。同じ構成のため
      // 予防的にmax_uses削減+max_tokens増量で対処。
      max_tokens: 16000,
