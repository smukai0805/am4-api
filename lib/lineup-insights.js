...
import { applyVerifiedLineupOverride } from './verified-lineup-overrides.js';
const cache = new Map();
...
export async function getLineupInsights(fixture, reader = read) {
  ...
  const settled = ['FT','AET','PEN'].includes(fixture.status);
  // Providers can correct the declared shape and player grids around kickoff.
  // Keep pre-match and live lineups fresh enough to replace a provisional XI;
  // finished matches can use the longer cache.
  const official = await safe(reader('/fixtures/lineups', {fixture:id}, settled ? 300000 : 60000));
  const confirmed = (official || []).filter(l => l.startXI?.length === 11);
  const lineups = confirmed.map(l => applyVerifiedLineupOverride(id, {...l,startXI:l.startXI.map(normalize),substitutes:(l.substitutes || []).map(normalize),predicted:false}));
  if (!upcoming) {
