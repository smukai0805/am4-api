...
function articleBlocks(article) {
  return articleContent.withoutAgenda(Array.isArray(article?.body)
    ? article.body : articleContent.parseMarkdownBlocks(article?.body));
}
...
function visibleArticleBlocks(article) {
  ...
  return reportPresentation.withEditorialArticleMotm(article, articleBlocks(article)).map((block) => {
...
function storedPlayerReference(card) {
  const playerId = positiveId(card?.playerId);
  const teamId = positiveId(card?.teamId);
  const photo = safeHttpUrl(card?.photoUrl || card?.photo);
  const logo = safeHttpUrl(card?.logoUrl);
  const playerName = text(card?.playerName).trim();
  const clubName = text(card?.clubName || card?.clubLabel).trim();
  const reason = text(card?.reason).trim();
  if (!card?.resolved || !playerId || !teamId || !photo || !logo || !playerName || !clubName || !reason) return null;
  return { ...card, playerName, playerId, teamId, photo, photoUrl: photo, reason, team: { id: teamId, name: clubName, logo } };
}

function renderEditorialMedia(article = {}) {
  if (article.type === 'match_prediction') {
    const cards = (Array.isArray(article?.prediction?.keyPlayerCards) ? article.prediction.keyPlayerCards : []).map(storedPlayerReference).filter(Boolean).slice(0, 2);
    if (!cards.length) return '';
    return `<section class="article-editorial-people" aria-label="キーマン">${cards.map((card) => playerCards.renderCard(card, { label: 'キーマン' })).join('')}</section>`;
  }
  if (article.type === 'match_report') {
    const card = storedPlayerReference(article?.report?.motmCard);
    if (!card) return '';
    return `<section class="article-editorial-people" aria-label="MOTM">${playerCards.renderCard(card, { motm: true, label: 'MOTM' })}</section>`;
  }
  return '';
}
export function renderArticleBody(article = {}) {
  ...
  return `<div class="article-body">${renderScoreboard(article.scoreboard)}${renderEditorialMedia(article)}${body}${sourceSection}</div>`;
}
...
function pageShell({ title, head, main, initialArticle = null, brandPill = 'AM4 COLUMN', includeAds = true }) {
...
<head>
  ...
  ${head}
  ${includeAds ? '<script async src="/api/adsense.js"></script>' : ''}
...
<body class="article-page">
  <header class="brand-topbar">
    ...
    <div class="article-top-actions" id="article-top-actions"><span class="brand-pill" id="article-brand-pill">${escapeHtml(brandPill)}</span></div>
  </header>
  ...
  <script src="/prediction-key-players.js?v=20260921-article-media-v1"></script>
  <script src="/match-report-presentation.js?v=20260918-brentford-motm-v8"></script>
  ...
  <script src="/article-page.js?v=20260918-brentford-motm-v8"></script>
</body>
</html>`;
}
...
export function renderArticlePage(article = {}) {
  ...
  return pageShell({ title, head, main, initialArticle: article, brandPill: articleTypeLabel(article.type) });
}
...
export function renderArticleErrorPage({ title, heading, message }) {
  ...
  return pageShell({ title, head, main, brandPill: 'AM4 Football', includeAds: false });
}
