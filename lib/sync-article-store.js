...
function compactStory(story) {
  ...
  return {
    ...
    relatedTeamIds: compactPositiveIds(story.relatedTeamIds),
    relatedPlayerIds: compactPositiveIds(story.relatedPlayerIds),
...
function compactPositiveIds(value) {
...
export function compactArticleIndexEntry(article = {}) {
  ...
  const compact = {
    ...
    // A failed creation is intentionally private until its Notion source is
    // edited. Retain only this compact monitor-owned marker in the index so
    // a full source scan does not lose the hold and republish the same body.
    ...
    relatedTeamIds: compactPositiveIds(article.relatedTeamIds),
    relatedPlayerIds: compactPositiveIds(article.relatedPlayerIds),
...
export function createSyncArticleStore({ blob = { get, put }, logger = console } = {}) {
  ...
  async function hasIndexedArticle(article) {
    if (!article?.id) return false;
    const current = await ensureIndex();
    const actual = current.find((entry) => entry?.id === article.id);
    return Boolean(actual && jsonEqual(actual, compactArticleIndexEntry(article)));
  }
