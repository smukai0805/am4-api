...
import motmTextPolicy from '../motm-text-policy.js';
...
【試合解説記事フォーマット(AM4)】(この見出し自体は記事本文に含めないこと。実際の記事は下記の指定フォーマット1.見出しから始めること)
...
6. 採点表(渡された採点データをそのまま使って出場選手を評価、Player of the Match選出、ボーナス加点箇所には補足コメントを表示)
7. 総括・今後の見どころ
...
- 採点(6.の採点表)は渡された算出済みデータをそのまま使うこと。記事内で独自に採点し直したり、
  渡された数値を変更したりしないこと
- MOTM/POTMは、公式または信頼できる媒体の選出を補足取材メモで確認できればそれを採用する。
  確認できない場合は、採点データのmomがある場合はAM4独自のMOTMとして必ず選出し、「AM4選出」と明記して理由を添えること。
  公式選出を装ってはならない。「記載しない」「選出しない」などの不掲載説明は本文に一切書かないこと
- Player of the Match・POTM・MOTM・MOMは同じ賞を指す。試合後は必ず1名を明記すること
- 採点表の直前に「## Player of the Match」という独立見出しを置き、その直下を
  「Player of the Match: 選手名」から始め、選出根拠となる主要スタッツを続けること
...
export function requirePlayerOfMatch(ratingResult) {
  if (!ratingResult?.mom?.name) throw new Error('Player of the Matchを確定できないため、試合解説記事を生成できません');
  return ratingResult.mom;
}
  ...
  for (const block of content || []) {
    if (block.type === 'web_search_tool_result' && Array.isArray(block.content)) {
      for (const result of block.content) {
        ...
        if (url) sources.push({ title: result.title || null, url });
      }
    }
  }
  return sources;
...
// Web検索を伴う呼び出しは、モデルが何回検索するか・どれだけ思考するかによって
// 所要時間のばらつきが非常に大きい(実データ検証で29秒〜258秒超まで変動を確認)。
// Vercelの実行時間上限(300秒)内に必ずwrite呼び出し分の時間を残すため、timeoutMsで
// 明示的に頭打ちにする(AbortController)。researchが失敗した場合は、根拠のない
// 下書きを作らないよう呼び出し元へ失敗を返す。
...
export async function generateMatchReportDraft(matchInfo, ratingResult, { client = callAnthropic } = {}) {
...
- 日時: ${matchInfo.date}
`;
  // 調査なしの試合解説を下書きとして残さない。失敗時はwatcherが候補を未処理のまま
  // 保つため、次回の正常な実行で安全に再試行できる。
  let researchNotes = '';
  let searchSources = [];
  try {
    const researchData = await client({
      maxTokens: 3000,
      tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 2 }],
      prompt: researchPrompt,
      label: 'research',
      timeoutMs: 150000, // 150秒で頭打ち(write呼び出し用の時間を必ず残すため)
    });
    ...
    if (!researchNotes.trim() || searchSources.length === 0) {
      throw new Error('research returned no usable notes or sources');
    }
  } catch (err) {
    console.error('research call failed; draft generation stopped:', err.message);
    throw new Error('取材情報を取得できなかったため、下書き生成を中止しました', { cause: err });
  }
...
`;
  const writeData = await client({
    maxTokens: 5000,
    tools: null, // ここではWeb検索を使わない(ツール往復を挟まず高速・確実に完走させるため)
    prompt: writePrompt,
    label: 'write',
    timeoutMs: 120000, // 120秒(Web検索なしのため通常は数十秒で完了する想定)
  });
  const rawDraft = (writeData.content || [])
    .filter(block => block.type === 'text')
    .map(block => block.text)
    .join('\n\n');
  // Prompt instructions are not a sufficient publishing boundary. Remove any
  // obsolete refusal sentence deterministically while preserving adjacent
  // player analysis; structured ratings still drive the AM4 fallback card.
  const cleanedDraft = motmTextPolicy.withoutAbstention(rawDraft);
  const fallbackSelection = motmTextPolicy.am4SelectionParagraph(ratingResult?.mom);
  const draft = fallbackSelection && !motmTextPolicy.hasPositiveSelection(cleanedDraft)
