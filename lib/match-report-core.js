...
`;
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      // 実データ検証でstop_reason:"max_tokens"により記事本文が生成される前に打ち切られる
      // 事象を確認した(Web検索5回分の結果で入力が20万トークン超になり、thinking+本文の
      // 生成分がmax_tokens内に収まらなかった)。max_uses削減+max_tokens増量で対処。
      max_tokens: 16000,
      tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 3 }],
      messages: [{ role: 'user', content: prompt }],
    }),
  });
  console.error(`[timing] anthropic fetch() resolved (headers only): ${Date.now() - tFetch}ms, status=${response.status}`);
  ...
  console.error(`[timing] anthropic response.json() parsed: ${Date.now() - tJson}ms, content blocks: ${(data.content||[]).length}, stop_reason: ${data.stop_reason}, usage: ${JSON.stringify(data.usage)}`);
