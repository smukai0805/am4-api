...
// `client` is injectable for local, side-effect-free verification. Production always
// uses the default Anthropic client.
export async function checkTransferNews({ client = callAnthropic } = {}) {
...
## 検索の進め方
以下の両方のアプローチを併用してください:
(a) 「site:x.com/FabrizioRomano」で本人のXプロフィールを直接検索する(プロフィール
    ページのピン留め投稿には、直近の独占情報がそのまま本文で表示されていることが
    ある)。ただし、この方法で表示される投稿は日付が古い過去の代表的な投稿(数年前の
    ものなど)が混ざっていることが多いため、投稿に添えられた日付を必ず確認し、
    直近数日以内のものだけを採用すること。個別の投稿URL(status/数字の形式)は
    ログイン要求等で本文が読めないことが多いので、無理に深追いしなくてよい。
(b) 「Fabrizio Romano transfer news」「Fabrizio Romano here we go」「Fabrizio Romano
    latest」のような検索語で、彼自身の発信を引用・報道しているニュースサイトの記事も
    探す。(a)のXプロフィール直接検索だけでは直近の投稿を十分に拾いきれないことが
    多いため、こちらを主な情報源として扱ってよい。
特定のクラブや選手名を起点に調べる必要はなく、ロマーノ自身の直近の発信を起点に
調べてください。
...
`;
  ...
  try {
    const researchData = await client({
      ...
      // 【2026-08-04追記】クレジット消費が大きいとの指摘を受け、実行頻度(8回/日→4回/日、
      // vercel.json参照)とあわせて検索上限も6→3に削減した(1日あたりのWeb検索上限は
      // 8×6=48件→4×3=12件、約4分の1に圧縮)。
      tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 3 }],
      prompt: researchPrompt,
      label: 'research',
      // 2026-08-04: 検索範囲を1トピックに絞ったことに伴い220000ms→120000msに短縮したが、
      // 実データ検証(クレジット補充後の実行)で、1トピックに絞った検索でも120000msに
      // 到達してタイムアウトすることを確認した(academy-core.jsのコメントにも「Web検索を
      // 伴う呼び出しは所要時間のばらつきが非常に大きく29秒〜258秒超まで変動」とあり、
      // 検索範囲を絞っても所要時間の変動幅自体は大きいままだった)。210000msまで
      // 引き上げた。write呼び出し(最大60000ms、実際はツール無しの純粋なテキスト生成の
      // ため通常もっと短い)を足しても最大270000msに収まり、Vercel関数の実行時間上限
      // (300秒、api/transfer-news-watch.jsのconfig参照)に対して30秒の余裕を残している
      // (research+writeの合計がこの上限ぴったりだと、内部のAbortControllerが発火する
      // 前にVercel自体に強制終了され、エラーメッセージすら残らない最悪のケースになるため)。
      timeoutMs: 210000,
    });
    ...
    // A failed research call must never fall through to the writing call. The
    // writer has no independently verified material in that case, so allowing it
    // to generate a public candidate would turn a transient provider failure into
    // unsupported reporting.
    return {
      items: [],
      rejected: [{
        description: '移籍情報の調査',
        reason: '調査取得に失敗したため、記事候補を生成しなかった',
      }],
      searchSources,
...
`;
  ...
  if (!match) {
    ...
    return { items: [], rejected: [], searchSources, researchStatus: 'available' };
  }
