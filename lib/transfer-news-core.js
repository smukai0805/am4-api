...
//
// 【2段階構成への変更(2026-07-31修正)】当初は検索(web_search)と分析・JSON整形を
// 1回のAI呼び出しにまとめていたが、実データ検証で、Web検索ツールが実際には検索結果
// (searchSources)を50件超取得できていたにもかかわらず、モデルがrejectedに「検索が
// エラーで一切取得できなかった」と記録し、items 0件で返す事例を確認した(プロンプトに
// 「部分的にでも結果があれば活用すること」という指示を追加しても再現し、解消しなかった)。
// 一部のクエリでのエラー(ツール利用上限超過等)が発生すると、モデルが同じ応答内で
// 既に得た検索結果まで巻き込んで「全滅」と結論づけてしまう挙動と見られる。
//
// match-report-core.js・academy-core.jsが既に採用している「検索(research、web_search
// あり)」→「分析・執筆(write、web_searchなし)」の2段階呼び出しに構成を合わせた。
...
以下のクラブに関係する、Fabrizio Romano(ファブリツィオ・ロマーノ)が直近数日以内に
報じた移籍情報をWeb検索で調べてください。
...
`;
  ...
  try {
    const researchData = await callAnthropic({
      maxTokens: 4000,
      // 2026-07-31実データ検証: max_uses:8だと15クラブ全体をカバーしきれず
      // (rejectedに「検索ツールの利用上限により全クラブ分の情報取得が完了せず」と
      // 記録された)、12に引き上げた。
      tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 12 }],
      prompt: researchPrompt,
      label: 'research',
      // 15クラブ分を横断的に調べるため、academy-core.js(1選手)より長めに確保。
      timeoutMs: 220000,
    });
...
- 調査メモ自体が「情報なし」の場合は、itemsを空配列にし、rejectedに「検索により
  有効な情報が得られなかった」旨を1件だけ記録すること(15クラブそれぞれについて
  個別に理由を書く必要は無い)
