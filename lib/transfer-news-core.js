...
`;
  ...
  console.error('[debug] transfer news raw parsed items:', JSON.stringify(items).slice(0, 2000));
  if (!Array.isArray(items)) return { items: [], searchSources };

  // sourceUrlが無い項目(出典不明)は速報化しない。
  const filtered = items.filter(item => item && item.player && item.toClub && item.sourceUrl);
  console.error(`[debug] transfer news items before/after filter: ${items.length}/${filtered.length}`);

  return { items: filtered, searchSources };
}
