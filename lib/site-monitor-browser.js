...
function runtimeFailureClass(message) {
  ...
  if (isTransientBrowserRuntimeFailure(text)) return 'runtime_browser_transport_interrupted';
  if (/executable|chromium.*not|launch.*failed/.test(text)) return 'runtime_launch_failed';
  return 'runtime_unclassified';
}

// These are Playwright/Chromium process or transport failures, not assertions
// about the reader-visible document. Keep this deliberately narrower than a
// generic "closed" or timeout match: a locator timeout can expose a genuine
// visual regression and must retain the normal failed/rollback path.
export function isTransientBrowserRuntimeFailure(message) {
  const text = String(message || '').toLowerCase();
...
function errorResult(error) {
  ...
  const unavailable = /cannot find package|module not found|executable|chromium.*not|launch.*failed|enoent/i.test(message)
    || isTransientBrowserRuntimeFailure(message);
  return unavailable
    ? {
