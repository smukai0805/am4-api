// lib/daily-digest-core.js(旧 api/daily-digest.js)
// 【2026-07 停止】画像のクロップが不自然になる問題、時間帯表示のズレなどの不具合があり、
// 直すより一旦停止する判断となったため、Cron呼び出し(vercel.json)とフロント側の表示を止めている。
// 【2026-07-31】Vercel Hobbyプランのサーバーレス関数数上限(12個/デプロイ)の枠を確保するため、
// api/直下からlib/配下に移設した(Vercelはapi/直下のファイルのみを関数としてカウントする)。
// 中身(ロジック)は変更していない。復活する場合は、api/daily-digest.js のような薄いラッパー
// (`export { default } from '../lib/daily-digest-core.js';`)を作るか、このファイルを
// api/直下に戻したうえで、vercel.jsonにcronsを戻し、football-hub.html側の
// initDailyDigest呼び出しを再度有効にすること。
//
...
// 2026-08-01: 元々 './news.js' を参照していたが、これはlib/news.js(存在しない)を
// 指す誤ったパスだった(このファイル自体が未使用のため実害は無かった)。api/news.jsの
// lib/news-core.jsへの移設に合わせて正しいパスに修正した。
import { fetchAllNewsItems, fetchWikipediaImage } from './news-core.js';
