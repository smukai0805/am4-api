// lib/news-core.js(旧 api/news.js)
//
// サッカー専門メディアのRSSフィード取得・パース、SNS埋め込みURL抽出、Wikipedia連携の
// 共有ヘルパー関数群。api/feature.js(「特集記事をリクエスト」機能)から使われる。
//
// 【2026-08-01、api/news.jsから移設】api/news.js自身のHTTPハンドラー(?mode=other・
// デフォルトの通常ニュース一覧の両方)は、フロントエンドから一度も呼び出されていない
// ことを実データ確認の上、確認した(?mode=otherを呼んでいた唯一の呼び出し元
// loadOtherNews()は既に2026-07-31時点で「未使用」とコメントされていた)。一方、
// このファイルがexportする関数(fetchAllNewsItems・attachEmbedUrls・
// fetchWikipediaImage・fetchWikipediaSummary)はapi/feature.jsから実際に使われて
// いたため、Vercel Hobbyプランの関数数上限(12個)対策として、HTTPハンドラー部分を
// 削除しlib/配下の共有モジュールとして残した(api/直下のファイルだけが関数として
// カウントされるため、lib/へ移すだけで関数数が1つ減る)。
//
...
// 取得できた生の記事一覧をそのまま返す。
// api/feature.js側でこの関数を再利用し、実際の報道記事をAIに読ませて根拠にする。
export async function fetchAllNewsItems() {
  const results = await Promise.allSettled(
    FEEDS.map(async feed => {
      // 1フィードあたり8秒でタイムアウトさせる。呼び出し元がこの関数の後に
      // さらにAnthropic APIを呼ぶことがあるため、1つのフィードが遅い/応答しないだけで
