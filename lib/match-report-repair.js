...
function positiveMotmComments(entry) {
  return (Array.isArray(entry?.comments) ? entry.comments : [])
    .map((value) => String(value || '').trim())
    .filter((value) => value && /(?:加点|無失点に貢献)/u.test(value) && !/^(?:警告|退場)/u.test(value));
}

function motmContributionWeight(entry) {
  return positiveMotmComments(entry).reduce((score, comment) => {
    let value = 1;
    if (/勝ち越し点|同点弾/u.test(comment)) value = 6;
    else if (/先制点/u.test(comment)) value = 5;
    else if (/ダメ押し点/u.test(comment)) value = 4;
    else if (/無失点に貢献/u.test(comment)) value = 2;
    if (/アシスト/u.test(comment)) value -= 1;
    return score + Math.max(1, value);
  }, 0);
}

function generatedMotm(ratingResult) {
  return (Array.isArray(ratingResult?.ratings) ? ratingResult.ratings : [])
    .filter((entry) => positiveMotmComments(entry).length)
    .sort((left, right) => (
      motmContributionWeight(right) - motmContributionWeight(left)
...
function appendAm4Motm(draft, ratingResult) {
  const mom = generatedMotm(ratingResult);
  const contributions = positiveMotmComments(mom);
  if (!mom?.name || !mom?.team || !contributions.length) {
    throw new MatchReportRepairError('A concrete verified match contribution is required for AM4 MOTM', {
      code: 'MOTM_SELECTION_UNAVAILABLE', retryable: false,
    });
  }
...
## 試合主要人物
MOTM：${mom.name}（${mom.team}）— AM4独自選出。
根拠：${contributions.slice(0, 2).join('。')}。`;
}
