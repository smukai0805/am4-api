(function () {
  ...
  function readInitialArticle() {
    if (!id) return null;
    const initial = document.getElementById('am4-initial-article');
    if (!initial) return null;
    try {
      const article = JSON.parse(initial.textContent || '');
  ...
  function appendBody(container, article) {
    const sourceBlocks = AM4ArticleContent.withoutAgenda(Array.isArray(article.body)
      ? article.body
    ...
    blocks.forEach((block) => {
      ...
      const visible = cleanText(block.text);
      if (!visible) return;
  ...
  function relatedTeamTags(article) {
    const seen = new Set();
    return (Array.isArray(article?.relatedTeams) ? article.relatedTeams : [])
      .map((team) => ({ id: Number(team?.id), name: compactArticleText(team?.name) }))
      .filter((team) => Number.isSafeInteger(team.id) && team.id > 0 && team.name)
      .filter((team) => {
  ...
  function renderArticleTags(container, article) {
    ...
    const teams = relatedTeamTags(article);
    if (!tags.length && !teams.length) return null;
    const section = document.createElement("section");
    ...
    tags.forEach((tag) => {
      ...
      const team = teams.find((candidate) => candidate.name === tag);
      link.href = team ? `/teams/${team.id}` : `/?columnTag=${encodeURIComponent(tag)}#lead-story`;
      link.textContent = displayArticleTag(tag);
      link.setAttribute("aria-label", team
        ? `${displayArticleTag(tag)}のチーム詳細を開く`
        : `${displayArticleTag(tag)}でAM4 COLUMNを絞り込む`);
      if (team) linkedTeamIds.add(team.id);
      list.append(link);
    });
    teams.filter((team) => !linkedTeamIds.has(team.id)).forEach((team) => {
      const link = document.createElement("a");
      link.className = "article-tag-link";
      link.href = `/teams/${team.id}`;
      link.textContent = displayArticleTag(team.name);
      link.setAttribute("aria-label", `${displayArticleTag(team.name)}のチーム詳細を開く`);
      list.append(link);
    });
  ...
  const initialArticle = readInitialArticle();
  if (initialArticle) renderArticle(initialArticle);
  else loadArticle();
})();
