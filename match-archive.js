(function (root, factory) {
  ...
  const TEAM_KEY_ALIASES = {
    ...
    // only: automatic repair still requires the ordered opponent and a unique
    // fixture inside the bounded ±48-hour window.
    brest: "stadebrestois29", stadebrestois: "stadebrestois29", stadebrestois29: "stadebrestois29",
    bodglimt: "bodoglimt", bodoglimt: "bodoglimt",
    ferencvaros: "ferencvarositc", ferencvarositc: "ferencvarositc",
    lillestrm: "lillestrom", lillestrom: "lillestrom",
    olympiacos: "olympiakospiraeus", olympiakos: "olympiakospiraeus", olympiakospiraeus: "olympiakospiraeus",
    gnkdinamo: "dinamozagreb", dinamozagreb: "dinamozagreb",
    // English and Czech forms of the same senior club. As with every alias,
    // this cannot bind an article by name alone: fixture resolution still
    // requires the ordered opposing club and one unique ±48-hour candidate.
    spartaprague: "spartapraha", spartapraha: "spartapraha",
    ...
    brighton: "brightonandhovealbion", brightonandhovealbion: "brightonandhovealbion", brightonhovealbion: "brightonandhovealbion",
    leeds: "leedsunited", leedsunited: "leedsunited",
    ipswich: "ipswichtown", ipswichtown: "ipswichtown",
    celta: "celtavigo", celtavigo: "celtavigo",
    rccelta: "celtavigo", celtadevigo: "celtavigo",
    ...
    // API-Football uses the formal 1899 label while Notion authors often use
    // the club's TSG name. Both identify the same senior Hoffenheim side; the
    // fixture resolver still requires an ordered opponent and unique ±48-hour
    // candidate before this alias can create a durable association.
    hoffenheim: "1899hoffenheim", tsghoffenheim: "1899hoffenheim", "1899hoffenheim": "1899hoffenheim",
    rcdeportivo: "deportivolacoruna", deportivolacoruna: "deportivolacoruna",
  };
  ...
  // City / Manchester United) is not enough. It is never a generic search:
  // callers add both clubs plus either a complete Match Key or a unique,
  // bounded kickoff window before accepting a result.
  ...
  // Compare ordered club identities independently from competition/date. It
  // is deliberately exported for the bounded fixture resolver, which adds a
  // unique ±48-hour window before treating this as a final recovery fallback.
  // Callers that do not impose those guards must use matchIdentityComparison.
  function orderedTeamIdentityComparison(left, right) {
  ...
  function matchIdentityComparison(left, right) {
    ...
    return orderedTeamIdentityComparison(leftParts, rightParts);
  }
