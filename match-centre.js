(function (root, factory) {
  ...
  function scoreDisplayParts(home, away, hidden) {
    // Use the same blurred silhouette for every hidden result: neither the
    // original digit shape nor a two-digit score may give the result away.
    return hidden ? ["8", "8"] : [String(home), String(away)];
  }
  ...
  function create({
    ...
    const revealedFixtureResults = new Set();
    ...
    const prefetchedMatchIds = new Set();
    let matchPrefetchObserver = null;
    let matchNavigationFeedbackTimer = null;
    ...
    function matchDetailHref(fixture) {
      const id = Number(fixture?.id);
      return Number.isInteger(id) && id > 0 ? `/match.html?id=${encodeURIComponent(id)}` : "";
    }

    function teamDetailHref(teamId, returnPath = "") {
      const id = Number(teamId);
      if (!Number.isInteger(id) || id <= 0) return "";
      const safeReturn = String(returnPath || "");
      return `/teams/${encodeURIComponent(id)}${safeReturn.startsWith("/") && !safeReturn.startsWith("//")
        ? `?return=${encodeURIComponent(safeReturn)}`
        : ""}`;
    }

    function matchTransitionApi() {
      return window.AM4MatchTransition || null;
    }

    function canPrefetchMatch() {
      const transition = matchTransitionApi();
      if (typeof transition?.shouldPrefetchMatch !== "function") return false;
      const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection || null;
      return transition.shouldPrefetchMatch({ prefetchedCount: prefetchedMatchIds.size, connection });
    }

    function prefetchMatch(fixture) {
      const id = Number(fixture?.id);
      const href = matchDetailHref(fixture);
      if (!href || !Number.isInteger(id) || id <= 0 || prefetchedMatchIds.has(id) || !canPrefetchMatch()) return false;
      const existing = document.head.querySelector(`link[data-am4-match-prefetch="${id}"]`);
      if (existing) {
        prefetchedMatchIds.add(id);
        return false;
      }
      ...
      link.dataset.am4MatchPrefetch = String(id);
      link.addEventListener("error", () => {
        prefetchedMatchIds.delete(id);
        link.remove();
      }, { once: true });
      prefetchedMatchIds.add(id);
    ...
    function scheduleMatchPrefetch(fixture) {
      if (!canPrefetchMatch()) return;
      const run = () => { prefetchMatch(fixture); };
    ...
    function observeVisibleMatchCards(cards) {
      if (matchPrefetchObserver) matchPrefetchObserver.disconnect();
      matchPrefetchObserver = null;
      if (!cards.length || !canPrefetchMatch()) return;
      const visibleCards = cards.filter(({ row }) => row?.isConnected && !row.hidden && !row.closest("[hidden]"));
      if (!visibleCards.length) return;
      if (typeof window.IntersectionObserver !== "function") {
        visibleCards.slice(0, 1).forEach(({ fixture }) => scheduleMatchPrefetch(fixture));
        return;
      }
      const fixtureByCard = new WeakMap();
      matchPrefetchObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting || entry.target.hidden || entry.target.closest("[hidden]")) return;
          const fixture = fixtureByCard.get(entry.target);
          observer.unobserve(entry.target);
          if (fixture) scheduleMatchPrefetch(fixture);
        });
      }, { root: null, rootMargin: "0px 0px 360px 0px", threshold: 0.01 });
      visibleCards.forEach(({ fixture, row }) => {
        fixtureByCard.set(row, fixture);
        matchPrefetchObserver.observe(row);
      });
    }

    function writeMatchTransitionSnapshot(fixture) {
      const transition = matchTransitionApi();
      if (typeof transition?.createMatchTransitionSnapshot !== "function") return null;
      const snapshot = transition.createMatchTransitionSnapshot(fixture);
      if (!snapshot || !transition.MATCH_TRANSITION_STORAGE_KEY) return null;
      try {
        sessionStorage.setItem(transition.MATCH_TRANSITION_STORAGE_KEY, JSON.stringify(snapshot));
        return snapshot;
      } catch (_error) {
        return null;
      }
    ...
    function isPlainMatchNavigation(event) {
      return !event.defaultPrevented && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.altKey
        && (event.button == null || event.button === 0);
    }

    function navigationFeedback() {
      let feedback = document.getElementById("match-navigation-feedback");
      if (feedback) return feedback;
      feedback = document.createElement("div");
      feedback.id = "match-navigation-feedback";
      feedback.hidden = true;
      feedback.setAttribute("role", "status");
      feedback.setAttribute("aria-live", "polite");
      const label = document.createElement("span");
      label.className = "sr-only";
      label.textContent = "試合詳細を開いています";
      const bar = document.createElement("span");
      bar.className = "match-navigation-feedback-bar";
      feedback.append(label, bar);
      document.body.append(feedback);
      return feedback;
    }

    function beginMatchNavigation(fixture, row) {
      if (!row || row.dataset.matchNavigationPending === "true") return;
      row.dataset.matchNavigationPending = "true";
      row.classList.add("fixture-row--opening");
      writeMatchTransitionSnapshot(fixture);
      prefetchMatch(fixture);
      const feedback = navigationFeedback();
      feedback.hidden = false;
      feedback.classList.add("is-visible");
      try { performance.mark("am4:match-card-feedback"); } catch (_error) { /* Performance API is optional. */ }
      if (matchNavigationFeedbackTimer != null) window.clearTimeout(matchNavigationFeedbackTimer);
      matchNavigationFeedbackTimer = window.setTimeout(() => {
        row.dataset.matchNavigationPending = "false";
        row.classList.remove("fixture-row--opening");
    ...
    function favoriteButton(type, id, label, className = "") {
      ...
      button.dataset.favoriteDetail = type === "clubs" ? `${label}のチーム詳細を確認` : `${label}の試合を優先表示`;
      button.dataset.favoriteHref = teamId ? teamDetailHref(teamId) : "#fixtures";
    ...
    function fixtureTeam(name, logo, score = "", teamId = null) {
      const href = teamDetailHref(teamId);
      const team = document.createElement(href ? "a" : "span");
      team.className = `fixture-team${href ? " fixture-team-link" : ""}`;
      if (href) {
        team.href = href;
        team.addEventListener("click", () => {
          // Persist the independent match-list state before navigating. The
          // link remains a real anchor for keyboard and touch users; this is
          // not a propagation-only workaround around the card overlay.
          persistState({ updateUrl: true, forceHash: "fixtures" });
          const returnPath = `${window.location.pathname}${window.location.search}${window.location.hash}`;
          team.href = teamDetailHref(teamId, returnPath);
        });
      }
    ...
    function renderFixtures(items, sourceLabel = "") {
      ...
      [...grouped.primary, ...grouped.other].forEach((groupInfo, currentGroupIndex) => {
        ...
        group.style.setProperty("--league-accent", isFavoriteGroup ? "#a9c5ff" : competitionAccent(groupFixture));
        ...
        fixtures.forEach((fixture, fixtureIndex) => {
          ...
          row.style.setProperty("--league-accent", competitionAccent(fixture));
          ...
          const fixtureRevealKey = String(fixture.id || [fixture.date, fixture.competition, fixture.home, fixture.away].join("|"));
          const locallyRevealed = revealedFixtureResults.has(fixtureRevealKey);
          const resultPresentation = AM4FootballData.fixtureResultPresentation(
            fixture,
            spoilersRevealed || locallyRevealed,
          );
          const isPerFixtureResultControl = !spoilersRevealed && ["live", "finished"].includes(statusGroup);
          ...
          status.className = `fixture-status fixture-status--${statusGroup}`;
          const phaseLabel = { "1H": "前半", HT: "前半終了", "2H": "後半", ET: "延長", BT: "延長前半終了", P: "PK戦", SUSP: "一時中断", INT: "中断" }[fixture.status];
          status.textContent = phaseLabel || fixtureStatusLabel(fixture.status);
          if (["1H", "2H", "ET", "LIVE"].includes(fixture.status) && fixture.elapsed != null) {
            status.textContent += ` ${fixture.elapsed}′`;
          }
          meta.append(status);
          if (isFavoriteGroup) {
            const competitionLabel = document.createElement("span");
            competitionLabel.className = "fixture-card-competition";
            competitionLabel.textContent = fixture.competition || "大会情報確認中";
            meta.prepend(competitionLabel);
          }
          ...
          const scoreboard = document.createElement(isPerFixtureResultControl ? "button" : "div");
          scoreboard.className = "fixture-scoreboard";
          if (isPerFixtureResultControl) {
            scoreboard.type = "button";
            scoreboard.classList.add("fixture-scoreboard--result-control");
            scoreboard.setAttribute("aria-pressed", String(locallyRevealed));
            scoreboard.setAttribute("aria-label", locallyRevealed
              ? `${fixture.home}対${fixture.away}、結果を隠す`
              : `${fixture.home}対${fixture.away}の試合結果を表示`);
          }
          ...
          const replaceScoreParts = (parts) => {
            scoreValue.replaceChildren();
            parts.forEach((part, index) => {
              if (index) {
                const separator = document.createElement("span");
                separator.className = "fixture-score-separator";
                separator.textContent = "−";
                scoreValue.append(separator);
              }
              const digit = document.createElement("span");
              digit.className = "fixture-score-digit";
              digit.textContent = part;
              scoreValue.append(digit);
            });
          };
          if (resultPresentation.hidden || (scoreText && ["live", "finished"].includes(statusGroup))) {
            scoreValue.classList.add("fixture-scoreboard-value--digits");
            replaceScoreParts(scoreDisplayParts(fullScores.home, fullScores.away, resultPresentation.hidden));
            if (resultPresentation.hidden) {
              scoreValue.setAttribute("aria-hidden", "true");
              scoreCaption.className = "fixture-score-reveal-hint";
              scoreCaption.textContent = "タップしたら試合結果を表示";
            } else if (isPerFixtureResultControl) {
              scoreCaption.className = "fixture-score-reveal-hint";
              scoreCaption.textContent = "タップしたら試合結果を表示";
              scoreCaption.hidden = true;
            }
          } else {
          ...
          if (isPerFixtureResultControl) {
            scoreboard.addEventListener("click", (event) => {
              ...
              const isHidden = scoreboard.dataset.resultHidden === "true";
              if (isHidden) {
                revealedFixtureResults.add(fixtureRevealKey);
                replaceScoreParts(scoreDisplayParts(fullScores.home, fullScores.away, false));
                scoreboard.dataset.resultHidden = "false";
                scoreboard.setAttribute("aria-pressed", "true");
                scoreboard.setAttribute("aria-label", `${fixture.home}対${fixture.away}、${scoreText}。結果を隠す`);
                scoreValue.removeAttribute("aria-hidden");
                scoreCaption.hidden = true;
              } else {
                revealedFixtureResults.delete(fixtureRevealKey);
                replaceScoreParts(scoreDisplayParts(fullScores.home, fullScores.away, true));
                scoreboard.dataset.resultHidden = "true";
                scoreboard.setAttribute("aria-pressed", "false");
                scoreboard.setAttribute("aria-label", `${fixture.home}対${fixture.away}の試合結果を表示`);
                scoreValue.setAttribute("aria-hidden", "true");
                scoreCaption.hidden = false;
              }
            });
          }
          ...
          if (fixture.id) {
            cardTarget.href = matchDetailHref(fixture);
            cardTarget.addEventListener("pointerdown", (event) => {
              if (isPlainMatchNavigation(event)) beginMatchNavigation(fixture, row);
            });
            // Older touch browsers without Pointer Events still get feedback
            // before the native anchor navigation begins.
            if (typeof window.PointerEvent !== "function") {
              cardTarget.addEventListener("touchstart", (event) => {
                if (isPlainMatchNavigation(event)) beginMatchNavigation(fixture, row);
              }, { passive: true });
            }
            cardTarget.addEventListener("pointerenter", (event) => {
              if (event.pointerType === "mouse") scheduleMatchPrefetch(fixture);
            });
            cardTarget.addEventListener("focus", () => scheduleMatchPrefetch(fixture));
            cardTarget.addEventListener("click", (event) => {
              persistState({ updateUrl: true, forceHash: "fixtures" });
              if (isPlainMatchNavigation(event)) beginMatchNavigation(fixture, row);
            });
            matchPrefetchCards.push({ fixture, row });
          }
          cardTarget.setAttribute(
            "aria-label",
            resultPresentation.hidden
              ? `${fixture.competition || "大会"}、${fixture.home}対${fixture.away}。${fixture.id ? "試合詳細へ移動" : "試合情報を開く"}`
        ...
        if (canCompact && !isExpanded) {
          ...
          showMore.addEventListener("click", () => {
            ...
            showMore.remove();
            observeVisibleMatchCards(matchPrefetchCards);
            persistState();
          });
    ...
    function roundOptions(data) {
      ...
      return rounds.map((round) => ({ value: round.key, label: round.label, small: "節別" }));
    }
    ...
    spoilerToggle?.addEventListener("click", () => {
      ...
      if (!spoilersRevealed) revealedFixtureResults.clear();
      spoilerToggle.setAttribute("aria-pressed", String(spoilersRevealed));
      spoilerToggle.querySelector("span").textContent = spoilersRevealed ? "結果を隠す" : "結果を表示";
