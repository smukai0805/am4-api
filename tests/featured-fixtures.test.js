...
test('featured fixtures prefer three different competitions when available', () => {
  const selected = selectFeaturedFixtures([
    ...
    fixture(3, 'ラ・リーガ', 541, 9003, '2026-08-22T18:00:00Z'),
    fixture(4, 'チャンピオンズリーグ', 529, 9004, '2026-08-23T18:00:00Z'),
  ]);

  assert.equal(selected.length, 3);
  assert.equal(new Set(selected.map((item) => item.competition)).size, 3);
});
