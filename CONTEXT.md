...
- **今日の試合**: the Japan-time daily schedule across every competition returned by the provider, including club friendlies. It is the homepage's primary route for answering what matches are on today; major competitions remain available as quick filters.
...
- **結果を隠す**: the default spoiler-safe state for completed matches. Live scores remain visible, while completed scores and goal scorers appear only after a visitor explicitly reveals them.
